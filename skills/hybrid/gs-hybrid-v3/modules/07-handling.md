...
| **上下文缺失** | CONTEXT_HYDRATION 未完成 | 强制完成注水 |
| **外部依赖** | 第三方服务不可用 | 降级或等待 |
| **任务中止** | 错误不可恢复或用户主动取消 | 转入 ABORTED 状态 |

---

## ABORTED 状态（任务中止）

**定义**: 任意状态均可转入 ABORTED，用于标记任务因不可恢复错误或用户主动取消而终止。

**触发条件**:
- 用户明确要求取消当前任务
- 遇到不可恢复的系统错误（如依赖完全不可用）
- 多次重试后仍无法通过关键 Gate
- 用户决定放弃当前方案

**中止流程**:
```
识别中止需求 → 记录中止原因 → 清理临时状态 → 通知用户 → 转入 ABORTED
```

**中止检查清单**:
- [ ] 记录中止原因和当前状态
- [ ] 保存已完成的工作产物（ADR、spec、plan 等）
- [ ] 清理临时文件和未提交的更改
- [ ] 通知用户中止完成及可恢复的入口

**从中止恢复**:
- 从 ABORTED 回到 IDEA 重新开始
- 已有的 Decision Layer 产物（ADR、spec）可复用，无需从头开始
- Context Layer 产物（契约文件）如未被破坏可直接复用
