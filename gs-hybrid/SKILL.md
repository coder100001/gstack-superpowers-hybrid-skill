---
name: "gs-hybrid"
description: "结合 Superpowers 方法论和 GStack 工程标准的混合流程。v3.8 更新：统一 debug 流程为 systematic-debugging-v2，引入 Bug 分级（Quick/Standard/Deep）；v3.7 更新：完整集成 48 个 GStack 技能。"
---

# Superpowers + GStack 混合流程 v3.8 (Debug 流程优化版)
...
> **v3.8 更新**：Debug 流程统一为 `systematic-debugging-v2`，引入 Bug 分级（Quick/Standard/Deep）和 Quick 路径 shortcut
...
| **GStack** | `skills/gstack/` | 48个 | 工程工具技能 |
...
| [systematic-debugging-v2](../superpowers/systematic-debugging/) | 系统调试 (统一版) | `/debug` 指令 |
...
| [openclaw](../gstack/openclaw/) | OpenClaw 集成 (子技能集合) |
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                 Superpowers + GStack 混合流程 v3.8 (模块化)                   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                               │
│   需求输入                                                                     │
│      │                                                                         │
...
| **Phase 1.5** | `plan-verification` | 所有任务 | Plan 验证确认 (范围/拆解/风险/验收硬阻断) — 内置于 `writing-plans` |
...
| **`/debug`** | `systematic-debugging-v2` | Bug/错误/测试失败 | 统一调试流程 (融合 Superpowers + GStack) |
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v3.8 | 2026-05-16 | **Debug 流程优化**：统一 `systematic-debugging-v2` skill，融合 Superpowers 方法论与 GStack 工具；引入 Bug 分级（Quick/Standard/Deep）和 Quick 路径 shortcut；消除 `systematic-debugging` 与 `/investigate` 的阶段重叠；替换 `brainstorming` 为 D1 Symptom Lock 专用模板 |
...
**版本**: v3.8 (Debug 流程优化版，统一 systematic-debugging-v2)
**最后更新**: 2026-05-16
